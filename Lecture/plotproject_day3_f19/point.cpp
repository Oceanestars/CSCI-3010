...
{
    // To change the signal and slot to have Point * parameters instead:
    // 1) update the PointClickedSlot method
    // 2) update PlotWindow.h (method definition), add include point.h
    // 3) update point.h  for the signal to carry Point * p parameter
    // 4) update point.cpp to emit PointSelected(this)
    ...
    if (PlotWindow::get_delete_mode()) {
        qDebug() << "plotwindow is in delete mode";
        emit DeleteMe(this);
    }
