...
void PlotWindow::PointClickedSlot(Point *p) {
    ...
    std::string p2 = "Point 2: (" + std::to_string(p->get_x()) + ", " + std::to_string(p->get_y()) + ")";
    QString s2(p2.c_str());
    ui->point2Label->setText(s2);
    }

    std::string p3 = "Distance:" + std::to_string(p->Distance(*p1data)) ;
    QString s3(p3.c_str());
    ui->distanceLabel->setText(s3);
}


bool PlotWindow::get_delete_point(){
return delete_mode_;
}
void PlotWindow::set_delete_point(){
